...
#
# Copied and modified from https://github.com/Rat-OS/RatOS-configurator
...
class RatRigHybridCoreXYKinematics:
    def __init__(self, toolhead, config):
        self.printer = config.get_printer()
        self.inverted = False
        if config.has_section('ratrig_hybrid_corexy'):
            hcxy_config = config.getsection('ratrig_hybrid_corexy')
        ...
        if config.has_section('dual_carriage'):
            ...
            self.dc_module = idex_modes.DualCarriages(dc_config, dc_rail_0, dc_rail_1, axis=0)
    ...
    def calc_position(self, stepper_positions):
        ...
        if (self.dc_module is not None and 'PRIMARY' == self.dc_module.get_status()['carriage_1']):
            if self.inverted == False:
...
def load_kinematics(toolhead, config):
    return RatRigHybridCoreXYKinematics(toolhead, config)
